...
async function test() {
  console.log('🧪 Testing Patreon scraper (limit: 5 creators)\n');

  const scraper = new PatreonScraper();
  const results = await scraper.discoverAndScrape(['recipe'], false, 2, 5);
  ...
  if (results.length > 0) {
    ...
    for (const result of results) {
      ...
      console.log(
        `   Subscription: ${result.subscription_price_lowest ? `$${result.subscription_price_lowest}/${result.subscription_currency}` : 'Free/Unknown'}`
      );
